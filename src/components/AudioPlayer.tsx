...
import { formatSecondsToMinutes, formatSongDuration } from '../utils/duration';
...
interface AudioPlayerProps {
  ...
  favorites: string[];
  onSongDurationDetected?: (songId: string, duration: string) => void;
}
...
export const AudioPlayer: React.FC<AudioPlayerProps> = ({
  ...
  const handleLoadedMetadata = () => {
    if (audioRef.current && audioRef.current.duration && !isNaN(audioRef.current.duration) && isFinite(audioRef.current.duration) && audioRef.current.duration > 0) {
      const realDuration = audioRef.current.duration;
      setDuration(realDuration);
      if (onSongDurationDetected && currentSong) {
        const formatted = formatSecondsToMinutes(realDuration);
        onSongDurationDetected(currentSong.id, formatted);
      }
    }
  };
